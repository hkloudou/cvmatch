...
`Match`, milliseconds, measured on 4-vCPU Intel Xeon @ 2.80 GHz (linux/amd64, cloud VM) (best of `-benchtime 5x`; native C++ is single-threaded because that
...
process per run, values from `docs/benchdata.json`): idle Go process
~12.9 MB, `cvmatch.Match` ~48 MB, `cvmatch.MatchGray` ~42 MB, native
OpenCV C++ ~166 MB. OpenCV materializes full double-precision integral
