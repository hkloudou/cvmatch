...
<picture>
  ...
  <img alt="Benchmark: native OpenCV C++ vs cvmatch (asm and no-asm builds) on amd64 and arm64, identical output" src="docs/bench-light.svg">
</picture>
...
**amd64** — GitHub Actions ubuntu-latest (4-vCPU INTEL(R) XEON(R) PLATINUM 8573C, linux/amd64):

| scene | native C++ | asm 1T | asm 4T | no-asm 1T | no-asm 4T |
|---|---|---|---|---|---|
| Window 1600×1000 · button 96×32 | 184.7 | 43.8 | **24.2** | 172.9 | 89.0 |
| Window 1600×1000 · icon 24×24 | 164.9 | 38.7 | **18.9** | 138.3 | 69.1 |
| Window 1600×1000 · panel 300×200 | 175.1 | 52.9 | **51.8** | 182.0 | 183.6 |
| Noise 1280×720 · sub 96×96 | 111.4 | 39.7 | **25.2** | 136.4 | 88.3 |
| Noise 1920×1080 · sub 128×128 | 291.2 | 69.6 | **44.8** | 263.1 | 146.0 |
| Noise 1920×1080 · sub 32×32 | 223.5 | 47.7 | **24.7** | 187.1 | 96.0 |
| Noise 640×480 · varying alpha, 4ch | 25.0 | 12.5 | **9.4** | 46.5 | 31.0 |
| Window 3840×2160 · button 96×32 | 895.4 | 242.1 | **120.1** | 873.9 | 447.4 |
| Noise 3840×2160 · sub 256×256 | 1289.3 | 334.6 | **187.8** | 1193.8 | 667.3 |
| fruits 512×480 · sub 80×80 | 45.1 | 6.9 | **6.5** | 21.0 | 21.7 |
| baboon 512×512 · sub 64×64 | 28.0 | 6.2 | **6.1** | 24.5 | 23.0 |
| building 868×600 · sub 100×100 | 74.9 | 23.3 | **21.7** | 75.5 | 72.9 |
| graf1 800×640 · sub 120×120 | 82.2 | **21.6** | **21.6** | 77.3 | 74.3 |
| starry_night 752×600 · sub 128×128 | 59.5 | **21.1** | 21.5 | 70.3 | 73.1 |

`MatchGray` at 1080p/128 for scale: asm 24.2 / 12.1 ms,
no-asm 88.2 / 49.6 ms (1T / 4T). Native C++ has no gray
...
**Measured summary (1T):** the asm build runs 3.0–4.0x
faster than the default no-asm build on amd64 (2.4–2.5x on
arm64), and beats native OpenCV C++ by 2.0–6.5x on amd64.
...
- The **default build** pays the SIMD multiple back (scalar complex
  ...
  on others single-threaded — read its own columns in the matrix rather
  than the headline. Internal threading pulls it back ahead of native on
  the big multi-tile scenes.
...
| minimal `Match` binary, `-ldflags "-s -w"` | **≈1.5 MB** in either build mode (a comparable OpenCV-linked Go binary measured 5.82 MB before that comparison was retired) |

Peak whole-process memory is probed per implementation in a fresh process
(VmHWM; the native probe is `native_bench … mem` — one end-to-end match,
the process reads its own peak) and the measured values are written into
the auto-generated matrix summary above on every refresh. The shape of
the result is structural, not incidental: OpenCV materializes full
double-precision integral images (~132 MB written per 1080p RGBA call),
while cvmatch replaces them with O(width) sliding sums, so its peak is
dominated by the input frame and the float32 response map themselves —
roughly a 3x lower peak than native on the same match.
